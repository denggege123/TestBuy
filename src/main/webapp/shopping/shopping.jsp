...
<body>
	<div id="shopping">
		<form action="javascript:buy()" method="post">
			<table id="myTableProduct">
				...
				<c:forEach items="${shoppingCart.shoppingCartItems }"
					...
					<tr class="productTr">
						...
						<td class="price"><span>￥<label
								id="productItem_total_${shoppingCartItem.product.id }">${shoppingCartItem.product.price*shoppingCartItem.count }</label></span>
						</td>
						<td class="number"><input type="hidden" id="product_id"
							value="${shoppingCartItem.product.id }" /> <input class="min"
							name="" type="button" value=" - " /> <input class="text_box"
							style="width: 30px; text-align: center" name="" type="text"
							value="${shoppingCartItem.count }" /> <input class="add" name=""
	...
	<div
		style="background-color: #cddbb8; margin-top: 10px; font-size: 20px; height: 40px; text-align: center">
